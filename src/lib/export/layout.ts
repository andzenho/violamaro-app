...
export function leadRequests(sheetId: number, dataRows: number, columns: number): unknown[] {
  ...
  if (dataRows > 0) {
    ...
    /* Порядок важен вдвойне: правила добавляются по index: 0, то есть каждое
       следующее встаёт первым, а красит строку первое подошедшее. Добавляем
       от менее важного к более важному, чтобы вышло: «нет контакта» →
       «повторная» → «свежая».

       Повторная выше свежей намеренно. Иначе вернувшийся человек в день
       своего возврата красился бы как обычная свежая заявка, и самый
       интересный случай — пришёл ещё раз — терялся бы ровно тогда, когда
       он и нужен. Свежие-однократные при этом остаются зелёными.

       «Нет контакта» перекрывает оба: это брак, а не оттенок. */
